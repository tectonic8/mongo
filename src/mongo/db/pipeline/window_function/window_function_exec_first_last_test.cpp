...
public:
    WindowFunctionExecFirst createForFieldPath(
        std::deque<DocumentSource::GetNextResult> docs,
        WindowBounds bounds,
        boost::optional<boost::intrusive_ptr<Expression>> defaultValue = boost::none) {
        auto docSource = DocumentSourceMock::createForTest(std::move(docs), getExpCtx());
        auto key =
            ExpressionFieldPath::parse(getExpCtx().get(), "$key", getExpCtx()->variablesParseState);
        _iter = std::make_unique<PartitionIterator>(getExpCtx().get(), docSource.get(), boost::optional<boost::intrusive_ptr<Expression>>(key));
        auto inputField =
            ExpressionFieldPath::parse(getExpCtx().get(), "$val", getExpCtx()->variablesParseState);
        return WindowFunctionExecFirst(
                    _iter.get(), inputField, bounds, defaultValue);
    }
...
TEST_F(WindowFunctionExecFirstLastTest, LookBehind) {
    const auto docs = std::deque<DocumentSource::GetNextResult>{
        ...
        Document{{"key", 0}, {"val", 2}},
    };

    // Look behind 1 document.
    auto fst = createForFieldPath(std::move(docs), {WindowBounds::DocumentBased{-1, 0}});
...
//     auto [fst, lst] = createForFieldPath(std::move(docs), {WindowBounds::DocumentBased{0, +1}});
//     ASSERT_VALUE_EQ(Value(0), fst.getNext());
//     ASSERT_VALUE_EQ(Value(1), lst.getNext());
//     advanceIterator();
//     ASSERT_VALUE_EQ(Value(1), fst.getNext());
